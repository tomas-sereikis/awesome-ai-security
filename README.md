...
* [whatileaked](https://github.com/selan-ai/whatileaked) - _Scans local Claude Code and Codex transcripts and memory files (CLAUDE.md, AGENTS.md) for credentials that were already sent to a model provider, using the gitleaks rule set. It reports a rule name and a one-way fingerprint instead of the credential, and can redact findings in place. No network calls, no telemetry, zero dependencies. MIT._
